...
class GoPowerupLevel(models.Model):
    level = models.DecimalField(
        db_column="Level", primary_key=True, max_digits=3, decimal_places=1
    )
    cp_multiplier = models.DecimalField(
        db_column="CP_Multiplier", max_digits=11, decimal_places=10
    )
...
class GoTrainerLevel(models.Model):
    ...
    unlocked_items = models.CharField(
        db_column="Unlocked_items", max_length=22, blank=True, null=True
    )
    ...
    log_xp = models.DecimalField(
        db_column="log_XP", max_digits=4, decimal_places=3, blank=True, null=True
    )
    log_total_xp = models.DecimalField(
        db_column="log_total_XP", max_digits=4, decimal_places=3, blank=True, null=True
    )
    max_xp = models.IntegerField(db_column="max_XP", blank=True, null=True)
    nanab_berries = models.IntegerField(
        db_column="Nanab_Berries", blank=True, null=True
    )
    pinap_berries = models.IntegerField(
        db_column="Pinap_Berries", blank=True, null=True
    )
